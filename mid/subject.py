...
class SubInfo:
    ...
    def __init__(self, config: dict, language: str = 'en'):
        ...
        self.lang_map = config.get('subinfo_lang', {}).get(language, {})
        ...
        if language not in config.get('subinfo_lang', {}):
            print(f"[SubInfo] WARNING: Language '{language}' not found in config. Falling back to English.")
            self.lang_map = config.get('subinfo_lang', {}).get('en', {})
        else:
            self.lang_map = config['subinfo_lang'][language]
